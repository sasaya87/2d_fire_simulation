...
double icp1 = 1.0 / cp1; //cp1の逆数
double cp2 = icp1 / (dx * dx);
double cp3 = icp1 / (dx * dx);
double cp4 = icp1 / (dy * dy);
double cp5 = icp1 / (dy * dy);
/*
...
double idy = 1.0 / dy;
double idt = 1.0 / dt;
double i3dx = 1.0 / (3.0 * dx);
double i3dy = 1.0 / (3.0 * dy);
double i12dx = 1.0 / (12.0 * dx);
double i12dy = 1.0 / (12.0 * dy);
...
void omega() {
    ...
    for ( i = 1; i < l + 2; i++) {
        for ( j = 1; j < m + 2; j++) {
            ...
            ome[i][j] = ((v[i + 1][j] - v[i - 1][j]) * idx - (u[i][j + 1] - u[i][j - 1]) * idy) * 0.5;
        }
    }
}
double vortCon(int i, int j, bool isU) {
    //double etax = (std::abs(ome[i + 1][j]) - std::abs(ome[i - 1][j])) / (2.0 * dx); //ηのx成分
    double etax = (std::abs(ome[i + 1][j]) - std::abs(ome[i - 1][j])) * 0.5 * idx; //ηのx成分
    //double etay = (std::abs(ome[i][j + 1]) - std::abs(ome[i][j - 1])) / (2.0 * dy); //ηのy成分
    double etay = (std::abs(ome[i][j + 1]) - std::abs(ome[i][j - 1])) * 0.5 * idy; //ηのy成分
...
void kawamuraByBiCGSTAB(int mode) {
    ...
    #pragma omp parallel
    {
        //cxはu,vで共通なのでvの時は計算せずにuの結果を流用
        if (mode != 1) {
            #pragma omp for private(j) collapse(2)
            for (i = 0; i < l + 3; i++) {
                for (j = 0; j < m + 3; j++) {
                    cx1[i][j] = idt + std::abs(u[i][j]) * 3.0 * 0.5 * idx + std::abs(v[i][j]) * 3.0 * 0.5 * idy
                        + tempc * 2.0 * (1.0 * idx * idx + 1.0 * idy * idy); //u3[i][j]の係数
                    double icx1 = 1.0 / cx1[i][j]; //cx1の逆数
                    cx2[i][j] = (-u[i][j] * 2.0 * i3dx - std::abs(u[i][j]) * idx - tempc * idx * idx) * icx1; //u3[i-1][j]の係数をcx1で割ったもの
                    cx3[i][j] = (u[i][j] * 2.0 * i3dx - std::abs(u[i][j]) * idx - tempc * idx * idx) * icx1; //u3[i+1][j]の係数をcx1で割ったもの
                    cx4[i][j] = (-v[i][j] * 2.0 * i3dy - std::abs(v[i][j]) * idy - tempc * idy * idy) * icx1; //u3[i][j-1]の係数をcx1で割ったもの
                    cx5[i][j] = (v[i][j] * 2.0 * i3dy - std::abs(v[i][j]) * idy - tempc * idy * idy) * icx1; //u3[i][j+1]の係数をcx1で割ったもの
                    cx6[i][j] = (u[i][j] * i12dx + std::abs(u[i][j]) * 0.25 * idx) * icx1; //u3[i-2][j]の係数をcx1で割ったもの
                    cx7[i][j] = (-u[i][j] * i12dx + std::abs(u[i][j]) * 0.25 * idx) * icx1; //u3[i+2][j]の係数をcx1で割ったもの
                    cx8[i][j] = (v[i][j] * i12dy + std::abs(v[i][j]) * 0.25 * idy) * icx1; //u3[i][j-2]の係数をcx1で割ったもの
                    cx9[i][j] = (-v[i][j] * i12dy + std::abs(v[i][j]) * 0.25 * idy) * icx1; //u3[i][j+2]の係数をcx1で割ったもの
        ...
        for (i = 2; i < l + 1; i++) {
            for (j = 2; j < m + 1; j++) {
                ...
                double xcon = ans[i][j] * idt + f; //cx1で割る前の右辺
...
void pressureByCG() {
    ...
    #pragma omp parallel
    {
        ...
        for (i = 2; i < l + 1; i++) {
            for (j = 2; j < m + 1; j++) {
                ...
                h = ((u3[i + 1][j] - u3[i - 1][j]) * idx + (v3[i][j + 1] - v3[i][j - 1]) * idy) * 0.5 * idt; //圧力のポアソン方程式の右辺
                resi[i][j] = h * icp1 - (cp3 * p[i + 1][j] + cp2 * p[i - 1][j] + cp5 * p[i][j + 1] + cp4 * p[i][j - 1] + p[i][j]);
                pp[i][j] = resi[i][j];
    ...
    while (continueFlag) {
        ...
        for ( i = 2; i < l + 1; i++) {
            for ( j = 2; j < m + 1; j++) {
                ...
                y[i][j] = pp[i][j] + cp2 * pp[i - 1][j] + cp3 * pp[i + 1][j] + cp4 * pp[i][j - 1] + cp5 * pp[i][j + 1];
                ppy += pp[i][j] * y[i][j];
            }
        }
    ...
    for ( i = 1; i < l + 2; i++) {
        for ( j = 1; j < m + 2; j++) {
            ...
            px[i][j] = (p[i + 1][j] - p[i - 1][j]) * 0.5 * idx;
            py[i][j] = (p[i][j + 1] - p[i][j - 1]) * 0.5 * idy;
        }
    }
    std::cout << cnt << std::endl;
}
